...
func ExtractContactsFromMessage(db *sql.DB, msg []string) error {
	...
	if len(msg) == 0 {
		logs.Logger.Error("error extracting contact statuses:", fmt.Errorf("empty message string"))
		return fmt.Errorf("empty message string")
	}

	contactStatuses, err := types.Convertmsgcontacts(msg)
	if err != nil {
...
func ExtractActivityFromMessage(db *sql.DB, msg []string) error {
	...
	activity, err := types.ConvertmsgActivity(msg)
	if err != nil {
